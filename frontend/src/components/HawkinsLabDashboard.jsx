...
import { getCurrentQuestion, getGameState, joinRoom, submitHawkinsHint } from '../lib/api';
import { getQuestionFor } from '../lib/questionBank';
...
export function HawkinsLabDashboard({ otherTeamProgress, onProgressUpdate }) {
  ...
  // Ensure this user is actually joined to the room as Team B.
  // If they manually set localStorage.roomCode (or refreshed), membership may not exist yet.
  useEffect(() => {
    ...
    (async () => {
      try {
        await joinRoom(roomCode, 'B');
      } catch {
        // ignore errors (e.g. already joined)
      } finally {
  ...
  // Poll points + current question from backend
  useEffect(() => {
    let mounted = true;
    ...
    fetchAll();
    const id = window.setInterval(fetchAll, 2500);
    return () => {
      mounted = false;
  ...
  const handleStageClick = (stageId) => {
    ...
    if (stage && dbPoints >= stage.unlockPoints) {
  ...
  const submitHint = async (answer) => {
    const roomCode = window.localStorage.getItem('roomCode');
    if (!roomCode) {
      console.warn('[HawkinsLab] Missing localStorage.roomCode; cannot submit hint.');
      return { correct: false };
    }
    const result = await submitHawkinsHint(roomCode, answer);
    // refresh current question state (so UI reflects "hint submitted")
    try {
      const q = await getCurrentQuestion(roomCode);
      setCurrentQuestion(q ?? null);
    } catch {
      // ignore
    }
  ...
  const isStageUnlocked = (stageId) => {
    const stage = stages.find((s) => s.id === stageId);
    return stage ? dbPoints >= stage.unlockPoints : false;
  };
    ...
    <div className="min-h-screen bg-[var(--void-black)] relative overflow-hidden">
      ...
      {selectedQuestion && (
        <QuestionModalLab
          ...
          title={
            getQuestionFor(currentQuestion?.round, 'B')?.title ??
            (currentQuestion?.round ? `ROUND ${currentQuestion.round}` : selectedQuestion.title)
          }
          questionText={
            (() => {
              const base =
                getQuestionFor(currentQuestion?.round, 'B')?.questionText ??
                selectedQuestion.questionText;
              if (currentQuestion?.team === 'B' && currentQuestion?.hintSubmitted) {
                return `${base}\n\n✅ Hint already submitted for this round.`;
              }
              return base;
            })()
          }
          points={currentQuestion?.points ?? selectedQuestion.points}
          isOpen={selectedQuestion !== null}
          onClose={() => setSelectedQuestion(null)}
          onSubmitAnswer={submitHint}
        />
      )}
    </div>
  );
