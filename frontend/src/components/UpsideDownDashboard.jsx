...
import { getCurrentQuestion, getGameState, submitUpsideDownAnswer } from '../lib/api';
...
export function UpsideDownDashboard({ otherTeamProgress, onProgressUpdate }) {
  ...
  // Poll points + current question/hint from backend
  useEffect(() => {
    let mounted = true;
  ...
  const submitAnswer = async (answer) => {
    const roomCode = window.localStorage.getItem('roomCode');
    if (!roomCode) {
      console.warn('[UpsideDown] Missing localStorage.roomCode; cannot submit answer.');
      return { correct: false };
    }
    const result = await submitUpsideDownAnswer(roomCode, answer);
    // Refresh points immediately after a correct submit
    try {
    ...
    <div className="min-h-screen bg-[var(--void-black)] relative overflow-hidden">
      ...
      {selectedQuestion && (
        <QuestionModal
          ...
          title={
            getQuestionFor(currentQuestion?.round, 'A')?.title ??
            (currentQuestion?.round ? `ROUND ${currentQuestion.round}` : selectedQuestion.title)
          }
          questionText={
            getQuestionFor(currentQuestion?.round, 'A')?.questionText ??
