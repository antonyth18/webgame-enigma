    ...
    <div className="min-h-screen bg-[var(--void-black)] relative overflow-hidden">
      ...
      <div className="relative z-10 px-12 py-16">
          ...
          <p className="text-lg text-[var(--ash-gray)] max-w-3xl mx-auto leading-relaxed">
            ...
            {otherTeamWorld && (
              <span className="block mt-4 text-[var(--ember-orange)]">
                Your rival team has entered:{' '}
                <span className="font-bold">
                  {worlds.find((w) => w.id === otherTeamWorld)?.title}
                </span>
              </span>
            )}
            {!otherTeamWorld && !isAuthenticated && (
              <span className="block mt-4 text-[var(--ember-orange)] font-semibold">
                Login required: Gateways remain sealed until your team is authenticated.
              </span>
            )}
          </p>
        </div>
        ...
        <div className="max-w-6xl mx-auto grid grid-cols-2 gap-8">
          {worlds.map((world) => {
            const isLockedByWorld = lockedWorld && lockedWorld !== world.id;
            const isLockedByAuth = !isAuthenticated;
            const isLocked = isLockedByWorld || isLockedByAuth;
                  ...
                  {!isLocked ? (
                    ...
                    <div className="flex flex-col items-center gap-3">
                      ...
                      <p className="text-sm text-[var(--ash-darker)] font-bold uppercase">
                        {isLockedByAuth ? 'Login Required' : 'Gateway Sealed'}
                      </p>
                    </div>
                  )}
                </div>
